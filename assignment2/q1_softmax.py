...
def softmax(x):
    ...
    ### YOUR CODE HERE
    max_values = tf.reduce_max(x,axis=1, keepdims=True)
    numerators = tf.exp(x - max_values)
    out = numerators / tf.reduce_sum(numerators,axis=1,keepdims=True)
    ### END YOUR CODE
...
def test_softmax_basic():
    ...
    arr = np.array([[123,325], [3,5],[14201240,401200]])
    test3 = softmax(tf.constant(arr, dtype=tf.float32))
    tf_softmax = tf.nn.softmax(
        tf.constant(arr, dtype=tf.float32))
    with tf.Session() as sess:
        test3 = sess.run(test3)
        tf_softmax  = sess.run(tf_softmax )
    test_all_close("Softmax test 3", test3, tf_softmax )
